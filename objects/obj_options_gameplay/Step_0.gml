...
for (var i = 0; i < array_length(options); i++) {
	...
	if !InputPlayerUsingGamepad(0) {
	    ...
	    if (my > rect.y && my < rect.y + rect.h && mx > rect.x && mx < rect.x + rect.w) {
	        ...
	        hovered_item = i;
			if item.name == "Show HUD"
				description_text = "If disabled, hides the HUD, great for cinematic screenshots";
			else if item.name == "Progress Display"
				description_text = "Shows how far in you are into a level at the bottom of the screen"
			else if item.name == "Skip Title Cards"
				description_text = "If enabled, skips the title card animation and jumps straight into the level"
			else if item.name == "Always Show Key Strokes"
				description_text = "A HUD element showing what keys are being pressed will always be shown at the top right instead of in certain events"
			else if item.name == "Level Background Dimming"
				description_text = "Dims the backgrounds inside levels to reduce distractions or blending in"
			else if item.name == "Show Leaderboard during Gameplay"
				description_text = "Shows a small leaderboard comparing your score and time to other users or GameJolt friends if linked to GameJolt"
			else if item.name == "Casual Mode"
				description_text = "Infinite lives, no losing bits upon getting hit, essentially the Easy mode of this game"
			else if item.name == "Show Collision Tags"
				description_text = "Areas and objects you can collide with will have their collision hitbox overlayed over itself"
			else if item.name == "Show Path Finders"
				description_text = "Sections of a level where paths are switched (plane switchers for example) will be shown"
			else if item.name == "Show Debug HUD"
				description_text = "Displays information about the player object's variables, such as X and Y coordinates, velocity, and more"
			else
				description_text = "Manage Sunrise's various Settings here!";
