...
for (var i = 0; i < array_length(options); i++) {
	...
	if !InputPlayerUsingGamepad(0) {
	    ...
	    if (my > rect.y && my < rect.y + rect.h && mx > rect.x && mx < rect.x + rect.w) {
	        ...
	        hovered_item = i;
			if item.name == "Tablet Mode"
				description_text = "Enables the virtual keyboard for desktop users (for example, if you're using a Surface Pro or drawing tablet with a screen)";
			else if item.name == "Raw Input"
				description_text = "Removes the post-processing Sunrise uses whenever taking input from your mouse";
			else if item.name == "Always Use Virtual Keyboard"
				description_text = "Enables the virtual keyboard no-matter what device you are using";
			else if item.name == "Haptics"
				description_text = "Certain events will vibrate the controller at different levels when enabled";
			else if item.name == "In-Game Button Guide"
				description_text = "Shows a guide to what buttons do what in menus at the bottom left of the screen";
			else if item.name == "Custom Light Bar Color"
				description_text = "Certain events will change the color of the light bar on your PlayStation 4 or PlayStation 5 controller";
			else if item.name == "Switch ACCEPT and CANCEL Mappings"
				description_text = "Much like PlayStation's Japanese controller layout, the ACCEPT and CANCEL buttons will be swapped around";
			else if item.name == "Calibrate Controller"
				description_text = "You can use this tool to calibrate the deadzones for your joystick on your controller, useful if you have stick drift";
			else
				description_text = "Manage Sunrise's various Settings here!";
