...
void main() {
	...
	float pr = smoothstep(-smoothness, 0.0, v.x * v_vTexcoord.x + v.y * v_vTexcoord.y - (d-0.5+progress * (1.0 + smoothness)));
	float s = step(pr, fract(count * (v.x * v_vTexcoord.x + v.y * v_vTexcoord.y)));
	gl_FragColor = v_vColour * mix(from_colour, to_colour, s);
}
